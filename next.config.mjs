...
const nextConfig = {
  ...
  async headers() {
    ...
    return [
      {
        // ⚠️ ВСЁ, КРОМЕ `/embed`. Запрет на чужие рамки ставится здесь, а не глобально:
        // embed-роут для того и существует, чтобы его вставляли к себе. Две записи с
        // разными CSP на один адрес дали бы ДВА заголовка, а браузер применяет их
        // пересечение — то есть самый строгий, и встраивание молча перестало бы работать.
        source: '/:path((?!.*embed).*)',
