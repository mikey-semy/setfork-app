...
> **Историческая справка (2026-07-30).** Переезд завершён: inproc-реализация и
> feature-flag `SETFORK_CORE_URL` удалены, ядро единственное. Документ оставлен как
> запись принятых тогда решений — актуальное состояние в `src/features/git/README.md`
> и HQ `tracks/git-format.md`.
