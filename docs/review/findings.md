...
| H2-005 | H2 | open | `src/features/git/list-content.ts:45` | toWireContent не шлёт danger/imageKey/needsHuman, а ядро переносит их только по block_id из ТЕКУЩЕЙ версии — у блока, которого там нет, пометки теряются молча |
