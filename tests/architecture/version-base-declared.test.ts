...
describe('гонка садовника не теряется в журнале', () => {
  ...
  it('каждая ветка пишет в журнал или возвращает исход наверх, где его запишут', () => {
    const mute = branches.filter((b) => !/journal\(|return\s*\{\s*result:\s*'stale'|return 'stale'/.test(b.body)).map((b) => b.rel)
