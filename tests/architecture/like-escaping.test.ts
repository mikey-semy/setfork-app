...
describe('экранирование шаблона поиска', () => {
  ...
  it('путь к помощнику ОДИН — второго входа нет', () => {
    // Первая версия правки оставила ре-экспорт в `features/library/queries/shared`, то
    // есть два адреса у одного правила. Ровно из-за такой раздвоенности оно и не
    // досталось трём соседним поверхностям: чтобы позаимствовать, надо было угадать, где
    // оно лежит. Один адрес — одно место, где смотреть.
    const reexports = walk(SRC)
      ...
      .flatMap((p) =>
        readFileSync(p, 'utf8')
          .split('\n')
          .map((line, i) => ({ line: line.trim(), n: i + 1 }))
          .filter(({ line }) => /export\s*\{[^}]*\blikeContains\b/.test(line))
          .map(({ line, n }) => `${p}:${n} ${line}`),
      )
