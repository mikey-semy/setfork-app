 ...
 *
 * Отметки «разобрано и законно» (`ui-parity-ok: <причина>` над местом) едут в тот же
 * слепок отдельной строкой и проверяются тем же храповиком. Это НАМЕРЕННО: отметка
 * снимает место с долга, то есть новая отметка — такое же изменение баланса, как
 * новый самопал, и обязана быть видна в дифференциале, а не появляться молча.
 */
