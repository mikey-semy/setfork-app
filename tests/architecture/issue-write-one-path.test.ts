...
describe('запись задач идёт одним путём', () => {
  ...
  it('⚠️ рассылка упоминаний в ядре ОДНА и спрашивает, кто пишет', () => {
    // Служба не обращается к людям — она цитирует чужой текст (садовник кладёт в задачу
    // битые ссылки ИЗ списка его автора). Развилка стояла у создания задачи, а путь
    // комментирования был закрыт лишь тем, что служебных комментаторов пока нет: то есть
    // отсутствием вызывающего, а не свойством. Узда держит свойство: точка рассылки в
    // ядре одна, и она за развилкой по писателю.
    const src = readFileSync('src/features/issues/core.ts', 'utf8')
    const calls = src.match(/await notifyMentions\(/g) ?? []
    expect(calls.length, 'вторая точка рассылки обойдёт проверку писателя').toBe(1)
    expect(src, 'и стоит она за развилкой').toMatch(/if \(writer !== 'person'\) return\s*\n\s*await notifyMentions\(/)
  })
