...
describe('запись задач идёт одним путём', () => {
  ...
  it('⚠️ точка рассылки упоминаний в ядре ОДНА', () => {
    ...
    // отсутствием вызывающего, а не свойством.
    //
    // Здесь проверяется ТОЛЬКО СТРУКТУРА — что путь рассылки один: второй в обход
    // развилки не появится незаметно. А то, что развилка работает, держат
    // ПОВЕДЕНЧЕСКИЕ тесты в tests/features/issues/service-writer.test.ts — на обоих
    // путях, создании и комментировании. Проверять здесь ещё и текст условия значило бы
    // закреплять способ записи: переименование `writer` или запись условия наоборот
    // красили бы верный код в красное, а тот же текст в незваной функции — в зелёное.
    const calls = readFileSync('src/features/issues/core.ts', 'utf8').match(/await notifyMentions\(/g) ?? []
