...
describe('подпись ветки', () => {
  ...
  it('имя, придуманное человеком, оставляет как есть', () => {
    ...
    // `u/<uuid>/fix-typo` — ветка ВЛАДЕЛЬЦА или соавтора: именованные ветки
    // разрешены только им, имя они выбрали сами, и прятать его нельзя.
    // Посторонний сюда не попадает — хук пускает его только в `refs/for/main`.
    for (const ref of [
      'main',
      'fix-typo',
      'v2.1_draft',
