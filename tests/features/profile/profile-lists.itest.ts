...
describe('выдача вкладок профиля', () => {
  ...
  describe('прежние правила не сломаны', () => {
    ...
    it('регистр не важен НА ОБОИХ языках — это свойство кластера, а не запроса', async () => {
      // ILIKE складывает регистр средствами ctype базы, и для не-ASCII это работает
      // только на UTF-8-локали. На кластере, поднятом с `--locale=C` (частый дефолт в
      // контейнерах), проверено: `'Хлеб' ILIKE '%хлеб%'` = false, а `'Bread' ILIKE
      // '%bread%'` = true — то есть поиск молча ломается ТОЛЬКО на русском, и на
      // английском корпусе выглядит исправным. Раньше отбор шёл в памяти, где регистр
      // складывал JS и от базы не зависел; перенос в SQL сделал локаль кластера
      // несущей. Утверждение здесь названо отдельно, чтобы неправильно поднятая база
      // падала понятной строкой, а не «поиск ничего не находит».
      expect(await slugs({ query: 'ХЛЕБОПЕЧКА' })).toEqual(['bread-baking'])
      expect(await slugs({ query: 'ХлЕбОпЕчКа' })).toEqual(['bread-baking'])
      // Английская половина той же проверки: она проходит и на сломанном кластере,
      // поэтому одна, без русской, ничего не доказывает.
      expect(await slugs({ query: 'BREAD' })).toEqual(['bread-baking'])
    })
