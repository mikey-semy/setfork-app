...
describe('публикация пачкой', () => {
  ...
  it('одобрение админа во время работы барьера не откатывается в очередь', async () => {
    ...
    //
    // Проверяем ИМЕННО модерацию, а не «очередь пуста»: публикация ставит ещё и
    // переиндексацию — опубликованный список обязан попасть в смысловой поиск. Прежняя
    // формулировка запрещала в очереди вообще всё и падала на законной джобе.
    const queued = (await db.select({ type: jobs.type }).from(jobs)).map((j) => j.type)
    expect(queued).not.toContain('moderate')
  })
