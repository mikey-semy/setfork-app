...
describe('parseSearchQuery', () => {
  ...
  it('ignores unknown/empty and bad values', () => {
    ...
    expect(p.text).toBe('foo is:open')
...
describe('is:verified снят', () => {
  it('не отбирает по флагу и НЕ теряет слово', () => {
    // ⚠️ ПОВЕДЕНИЕ ИЗМЕНЕНО ОСОЗНАННО, и вот почему. Раньше `is` оставался в списке
    // ключей без своего обработчика — и токен ПРОГЛАТЫВАЛСЯ: сохранённая ссылка
    // `?q=is:verified` молча возвращала весь корпус, то есть снятый отбор превращался в
    // «показать всё». Человек при этом видел выдачу, которую не просил.
    //
    // Теперь `is` не ключ вовсе, и токен ведёт себя как любой незнакомый (`foo:bar`) —
    // уходит в свободный текст. Старая ссылка ищет эти слова и находит их или ничего;
    // молчаливой подмены выдачи не остаётся ни в одном случае.
