...
describe('зеркало отстало без ошибки', () => {
  it('подметальщик берёт его, хотя ошибки нет', async () => {
    // Правка ВЫЛЕЖАЛАСЬ (льгота считается от неё), а синхронизация ещё старше.
    await laggingMirror('lag', 60, 30)
  ...
  it('отставшее, но правленное только что, ждёт паузу', async () => {
  ...
  it('давно не синхронизированное + правка секунду назад — тоже ждёт', async () => {
    // Тот самый случай, который проба выше НЕ ловила (замечание авто-ревью на #825):
    // у зеркала, которое не трогали часами, лестница пауз давно истекла, и льгота,
    // считаемая от СИНХРОНИЗАЦИИ, пропускала бы проход прямо поверх ядра, всё ещё
    // придерживающего пуш своим окном. Льгота обязана считаться ОТ ПРАВКИ.
    await laggingMirror('idle-then-edit', 180, 0)
    await sweepFailedMirrors()
    expect(pushed).toEqual([])
  })
})
