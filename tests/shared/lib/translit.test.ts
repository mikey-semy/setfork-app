import { describe, expect, it } from 'vitest'
import { detectTextLang, translitRu } from '@/shared/lib/translit'
import { slugify } from '@/features/library/slug'

describe('slugify с транслитерацией', () => {
  it('русский заголовок → латинский слаг (раньше давал «-»)', () => {
    expect(slugify('Домашнее маршмеллоу')).toBe('domashnee-marshmellou')
    expect(slugify('Топ-5 книг по Rust')).toBe('top-5-knig-po-rust')
  })

  it('латиница как раньше; пусто → list', () => {
    expect(slugify('My Cool List!')).toBe('my-cool-list')
    expect(slugify('!!!')).toBe('list')
  })
})

describe('detectTextLang', () => {
  it('кириллица → ru, латиница → en, смесь по доле', () => {
    expect(detectTextLang('Домашнее маршмеллоу')).toBe('ru')
    expect(detectTextLang('Homemade marshmallow')).toBe('en')
    expect(detectTextLang('Setup nginx на сервере с нуля')).toBe('ru')
    expect(detectTextLang('')).toBe('en')
    expect(detectTextLang('123 456')).toBe('en')
  })
})

describe('translitRu', () => {
  it('щ/ю/я и твёрдый знак', () => {
    expect(translitRu('Щука объелась')).toBe('schuka obelas')
  })
})

describe('таблица транслитерации заморожена', () => {
  // Каждая буква — ЯВНЫМ литералом, а не выводом из самой таблицы: иначе тест согласился
  // бы с любой её правкой. Схема своя (не ГОСТ, не ICAO); её смена меняет адреса списков
  // и ники — поэтому любое изменение здесь должно покраснеть.
  const TABLE: [string, string][] = [
    ['а', 'a'], ['б', 'b'], ['в', 'v'], ['г', 'g'], ['д', 'd'], ['е', 'e'], ['ё', 'e'], ['ж', 'zh'],
    ['з', 'z'], ['и', 'i'], ['й', 'y'], ['к', 'k'], ['л', 'l'], ['м', 'm'], ['н', 'n'], ['о', 'o'],
    ['п', 'p'], ['р', 'r'], ['с', 's'], ['т', 't'], ['у', 'u'], ['ф', 'f'], ['х', 'h'], ['ц', 'ts'],
    ['ч', 'ch'], ['ш', 'sh'], ['щ', 'sch'], ['ъ', ''], ['ы', 'y'], ['ь', ''], ['э', 'e'], ['ю', 'yu'],
    ['я', 'ya'],
  ]

  it.each(TABLE)('%s → «%s»', (ru, lat) => {
    expect(translitRu(ru)).toBe(lat)
    expect(translitRu(ru.toUpperCase())).toBe(lat)
  })

  it('весь алфавит — ровно 33 буквы, ни одна не пропущена', () => {
    expect(TABLE).toHaveLength(33)
    expect(translitRu(TABLE.map(([ru]) => ru).join(''))).toBe(TABLE.map(([, lat]) => lat).join(''))
  })

  it('буквы вне русского алфавита не транслитерируются — таблицу не расширяли', () => {
    // Украинские и белорусская: добавить их в таблицу — сменить слаги у таких заголовков.
    expect(translitRu('іїєґў')).toBe('іїєґў')
  })

  it('не кириллица — как есть (в нижнем регистре)', () => {
    expect(translitRu('Rust 2024 — ok')).toBe('rust 2024 — ok')
  })
})
