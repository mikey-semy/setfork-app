...
describe('keyset: курсор', () => {
  ...
  it('форма без календаря — не ключ: невозможная дата уронила бы `::timestamptz`', () => {
    // Строка выше `'2026-13-99 99:99:99+00 '` отсеивалась ХВОСТОВЫМ ПРОБЕЛОМ, а не
    // диапазоном: без пробела она проходила форму и падала уже в базе — 500 вместо
    // «покажем сначала» (а в MCP — текст SQL у клиента).
    for (const bad of [
      '2026-99-99 99:99:99+00',
      '2026-13-01 00:00:00+00',
      '2026-02-30 00:00:00+00',
      '2026-08-18 24:00:00+00',
      '2026-08-18 19:60:00+00',
      '2026-08-18 19:02:60+00',
      '2026-08-18 19:02:03+16',
      '0000-01-01 00:00:00+00',
    ]) {
      expect(decodeCursor(btoa(`${bad}~${ID}`)), bad).toBeNull()
    }
    for (const good of ['2024-02-29 23:59:59.999999+00', '2026-08-18 19:02:03-15:59', '2026-08-18 00:00:00+05:30']) {
      expect(decodeCursor(btoa(`${good}~${ID}`)), good).toEqual({ key: good, id: ID })
    }
  })
