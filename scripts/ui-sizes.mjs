...
const NAMED = new Map([
  ...
  [
    ...
    'ВКЛАДКИ внутри поповера по образцу WAI-ARIA: aria-controls на панели, стрелки, ' +
      'roving tabindex. Ни один примитив сюда не подходит по роли: TabNav — навигация ' +
      'ссылками по маршрутам, SegmentedControl (заведён 27.08.2026) — выбор ВИДА одного ' +
      'и того же, панелей он не переключает и клавиатурный контракт вкладок не несёт. ' +
      'Высота при этом на шкале: min-h-8 плюс pointer-coarse:min-h-11. Единственное такое ' +
      'место на весь код — примитив вкладок заводим, когда появится второе.',
  ],
])
