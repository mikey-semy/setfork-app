...
const ROLES = [
  ...
  },
  {
    ...
    hint: 'прописная метка группы («О СПИСКЕ», «ФАЙЛЫ») написана руками: свой кегль, свой цвет, своя разрядка',
    /**
     * Признак: ПРОПИСНЫЕ вместе с разрядкой. Ничем другим этот приём не пользуются —
     * обычный текст не разряжают, а прописные без разрядки в интерфейсе не ставят
     * (буквы слипаются). Примитив есть с Ф4 и зовётся из семнадцати мест.
     *
     * ⚠️ Одних прописных с разрядкой МАЛО: тем же приёмом пользуется бейдж
     * (`<Badge className="uppercase tracking-wide">`) — а это метка-плашка, у неё своя
     * роль и свой примитив. Поэтому спрашиваем и остальное: кегль подписи или
     * основного текста и приглушённый цвет. Иначе замер снова завысил бы долг,
     * а это уже шестой раз (K41* в карте корней).
     */
