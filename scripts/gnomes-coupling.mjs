 ...
 *
 * То есть текущее значение — правда про первый сценарий и НЕПРАВДА про второй. Считая
 * иначе, поменяйте `IS_SURFACE` и пересчитайте: цифра без сценария ничего не значит.
 *
 * ⚠️ САМОГО ЧИСЛА В ЭТОМ ТЕКСТЕ НЕТ намеренно. Оно живёт в одном месте — в слепке
 * `gnomes-coupling-baseline.json`, — и объяснение его не дублирует: назвав цифру здесь,
 * мы завели бы вторую копию правды, которая устаревает при первом же пересчёте. Так и
 * вышло: замер уже показывал другое число, а этот абзац продолжал называть прежнее.
 *
