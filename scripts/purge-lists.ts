...
async function main() {
  ...
  for (const r of target) {
    try {
      await db.delete(templates).where(inArray(templates.id, [r.id]))
      done++
    } catch (e) {
      failed.push({ id: r.id, slug: r.slug, error: e instanceof Error ? e.message : String(e) })
    }
  }
  // Аудит пишется в ту же таблицу теми же полями, но НЕ через `recordAuditMany`:
  // тот модуль помечен `server-only` и из скрипта не грузится. Действие называется
  // так же (`list.delete`), плюс метка `via`, чтобы пакетную зачистку можно было
  // отличить от удаления руками владельца.
  const survivors = target.filter((r) => !failed.some((f) => f.id === r.id))
  if (survivors.length) {
