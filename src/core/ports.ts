...
export interface CreateListInput {
   ...
   * на котором писала модель, форк — язык источника, MCP — аргумент. Не задан — `langFallback`.
   */
  lang?: string | null
  /**
   * Язык определяется ТОЛЬКО содержимым: не задан `lang` — языка нет, запасные варианты не
   * берутся. Для копий (форк, «как шаблон»): текст скопирован как есть и написан на языке
   * источника, а не на языке того, кто копирует.
   */
  langFromContent?: boolean
  /** Последний запасной вариант: язык, на котором автор пишет (интерфейс), или догадка по тексту. */
...
export interface ListBlock {
  ...
  /** Разрушительный пункт (из канона ветки). На ЗАПИСИ поле ЗНАЧИМО: ядро
   *  переносит пометку из текущей версии по blockId, но только для блока, который
   *  в той версии есть, — у блока, заведённого в ветке, переносить нечего.
   *  Снятое приходит пустым и переносу не мешает: ядро пишет в канон только
   *  непустое, так что клиент, пометку не заполнивший, её и не снимет. */
  danger?: boolean
   ...
   *  пометку. На записи значимы ровно как `danger`. */
  imageKey?: string
  needsHuman?: boolean
...
/** Коды отказа СПИСКОМ, а не только типом: по ним узда проверяет, что у каждого
 *  есть текст для человека. Тип выводится отсюда — разойтись им нечем. */
export const LIST_WRITE_CODES = ['stale', 'out-of-sync', 'exists', 'archived', 'frozen'] as const
export type ListWriteCode = (typeof LIST_WRITE_CODES)[number]

/** Запись списка отклонена по предусловию (ядром либо барьером фасада).
 ...
 *  'exists' — адрес списка уже занят. Отказ РОЖДЕНИЯ, а не правки: заведён 27.08.2026,
 *  когда вертикаль «собрать список» показала, что этот путь причины не имел вовсе.
 *  'archived' / 'frozen' — список read-only, новых версий у него не бывает. Отказ
 *  такой же по предусловию, но приходит не от ядра, а от барьера на единой точке
 *  записи версии (`assertVersionAllowed`). Раньше барьер бросал безымянный `Error`,
 *  а экшены ловят `ListWriteError` — и «Вернуть версию» с «Принять правку» на
 *  архивном списке кончались страницей ошибки без единого слова о причине.
...
export class BranchOpError extends Error {
  constructor(
    public code:
      ...
      | 'out-of-sync'
      // Спросить приложение о праве на запись НЕ УДАЛОСЬ, и два этих случая
      // требуют от человека РАЗНОГО (разведены ядром 25.08.2026, до этого оба
      // ехали как 'internal' с одним текстом «попробуйте ещё раз»):
      //  • связь с проверкой сорвалась — повтор осмыслен;
      | 'gate-unavailable'
      //  • проверка ответила непонятно: разошлись версии, и повтор бесполезен —
      //    человеку нужно не «ещё раз», а знать, что чинить будут без него.
      | 'gate-malformed'
      | 'internal',
  ) {
    super(code)
