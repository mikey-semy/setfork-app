...
/** Отказ стража содержимого для показа: вид, код причины и шаг (строкой — как из адреса). */
...
export function contentRefusalOf(e: unknown): ContentRefusal | null {
  if (e instanceof DestructiveCommandError) return { kind: 'destructive', reason: e.reason, step: String(e.stepIndex) }
  if (e instanceof SecretFoundError) return { kind: 'secret', rule: e.match.rule, step: String(e.stepIndex) }
  return null
}
