...
export type AuditAction =
  ...
  | 'passkey.remove'
  | 'passkey.login'
  // Ключ подтверждён, но аутентификатор пользователя НЕ проверил, а 2FA включена:
  // вход не завершён, ждём код. Отдельным событием, а не 'passkey.login', — иначе
  // в журнале незавершённый вход неотличим от состоявшегося.
  | 'passkey.login-needs-2fa'
  | 'account.delete'
