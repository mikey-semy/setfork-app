...
export const aiFeature = pgEnum('ai_feature', ['generate', 'regenerate', 'refine', 'note', 'moderate', 'embed', 'translate', 'mcp-gnome', 'dig', 'assist', 'gate', 'landing', 'decide'])
/**
...
export type ProposedItem = {
  ...
  blockId?: string
  /** ЯЗЫКОВАЯ ОБЛАСТЬ ЗАПИСИ: «здесь сказано только про этот язык, про остальные
   *  не сказано ничего». Ставит одноязычный редактор (toProposedItems); запись,
   *  которая распоряжается всем LocaleText сразу — MCP, перевод, импорт, — её не
   *  ставит и потому вправе язык УДАЛИТЬ. Без этой метки перенос переводов
   *  воскрешал бы удалённый язык (поймано интеграционным тестом patch_list).
   *  Транспортное поле: до ядра не доезжает, снимается фасадом listStore. */
  langScope?: string
  title: LocaleText
  desc: LocaleText
  command: string
...
export const JOB_TYPES = [
  ...
  'aiwatch',
  'mirror',
  // Эффекты ПРИНЯТОГО git-пуша: предложение из магической ветки, уведомления, аудит,
  // пере-проверка модерацией. Отдельной задачей, потому что держать ими ответ протокола
 ...
 *
 * Не одна отметка «когда был прошлый проход», а факт отправки по КАЖДОМУ списку, с тем, ЧТО
 * отправлено. Адрес списка меняется без правки самого списка: смена ника, переход списков
 * к «призраку» при удалении аккаунта, смена хоста; публичным список становится сменой
 * видимости и одобрением модерации — и ни один из этих путей `updated_at` не двигает. С
 * отметкой по времени правки такие списки не ушли бы никогда: ровно корень «забыли один
 * путь», от которого проход и заведён. Поэтому список уходит снова, если:
 *  - строки нет (ещё не отправлялся или открыт заново — при снятии публичности строка
 *    удаляется после сообщения об этом);
 *  - `updated_at` новее отправленного;
 *  - адрес или набор языков не тот, что отправлен (`sentUrl`, `sentLangs`).
 *
 * `sentUpdatedAt` — `updated_at` СПИСКА на момент чтения, а не время отправки: правка,
 * пришедшая пока пачка летела, уйдёт ещё раз. Пишется из текстовой формы с полной
 * точностью: JS-`Date` режет микросекунды, и «правился после отправки» было бы истинным
 * всегда.
 */
...
/**
 * ЗАЯВКИ «ХОЧУ PRO» — ЗАМЕР СПРОСА БЕЗ ПЛАТЁЖКИ (решение 0021).
 *
 * Fake door: кнопка тарифа ведёт к форме почты, биллинга за ней нет и не заявлено.
 * Смысл — арифметика, а не воронка: платёжная инфраструктура стоит недели, а вопрос
 * «захочет ли кто-то платить» решается двадцатью заявками. Порог на смену курса назван
 * в решении заранее (≥ 20), чтобы результат нельзя было истолковать задним числом.
 *
 * ⚠️ ФОРМА ОБЯЗАНА ГОВОРИТЬ, ЧТО ОПЛАТЫ ПОКА НЕТ. Иначе это не замер, а обещание,
 * которого мы не выполняем: человек уходит ждать письма о запуске, а мы считаем его
 * готовность платить. Текст формы — часть механизма, не украшение.
 *
 * `userId` может быть пуст: интерес выражают и незалогиненные. Почта — контакт для
 * ответа, покрыта тем же «Support data» в политике, что и обратная связь.
 */
export const proInterest = pgTable(
  'pro_interest',
  {
    ...
    /** Откуда нажали: у какого ограничения человек оказался. Это и есть замер спроса. */
    source: text('source').notNull().default(''),
    /** Что именно нужно, своими словами. Пусто — законно: заявка сама по себе сигнал. */
    note: text('note').notNull().default(''),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => [
