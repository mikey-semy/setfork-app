...
/** Цель в РЯДУ ОДНОТИПНЫХ, которая обязана дорасти по обеим сторонам, но чья ширина
 *  заранее не известна: шаг листалки бывает и квадратным значком, и значком с подписью.
 *  `TOUCH_BOX` тут не годится — он ЗАДАЁТ размер и раздавил бы вариант с подписью;
 *  `TOUCH_HIT` не годится тоже — он растит зону только вверх и вниз, а соседи здесь
 *  стоят сбоку, то есть узкая цель так и остаётся узкой. */
export const TOUCH_MIN_BOX = 'pointer-coarse:min-h-11 pointer-coarse:min-w-11'
