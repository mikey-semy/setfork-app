...
/**
 * СТРАНИЦА ЗА ПОСЛЕДНЕЙ — ЭТО 404, А НЕ ПОСЛЕДНЯЯ.
 *
 * `pageFromParam` зажимает номер к существующему: `?page=999` при трёх страницах
 * показывает третью. Для человека это удобно, а для обходчика — дубль: одно и то же
 * содержимое под бесконечным числом адресов, и canonical у каждого свой. Так это и
 * решают GitHub и Gitea: за последней страницей — «не найдено».
 *
 * Возвращает номер, если он существует, и `null`, если нет; решение о `notFound()`
 * принимает страница — этот модуль про арифметику листания, а не про ответы HTTP.
 *
 * ⚠️ ПЕРВАЯ СТРАНИЦА СУЩЕСТВУЕТ ВСЕГДА, даже когда показывать нечего: пустая полка или
 * тег без списков — это пустая страница, а не отсутствующая. Иначе новый тег отдавал бы
 * 404 до первого списка на нём.
 */
export function pageOrNotFound(raw: string | undefined, totalPages: number): number | null {
  // Слева край НЕ отказывает: `?page=0` и `?page=-2` — мусор в ссылке, и таких адресов
  // конечное число, а canonical у них всё равно голый путь (`canonicalPageParam`
  // приводит к единице). Отказываем только СПРАВА: там адресов бесконечно много, и
  // каждый показывал бы последнюю страницу под своим каноническим адресом.
  const n = Math.max(1, Math.floor(Number(raw)) || 1)
  return n <= Math.max(1, totalPages) ? n : null
}
...
/**
 * Номер страницы ДЛЯ CANONICAL — из адреса, без сверки с ЧИСЛОМ СТРАНИЦ.
 *
 * Границ здесь две, и путать их нельзя. Санитарный потолок (`MAX_PAGE`) стоит: без него
 * `?page=1e999` давал canonical `?page=Infinity`. А вот СКОЛЬКО СТРАНИЦ на самом деле,
 * знает только сама страница — она уже посчитала, — и метаданные считаются раньше,
 * отдельным проходом: повторять там счёт значило бы второй запрос к базе на каждый показ
 * ради краевого случая.
 *
 * Расхождения «страница показывает третью, canonical называет 999-ю» больше нет: у всех
 * трёх поверхностей с листалкой и canonical — тег, полка, профиль — за последней
 * страницей стоит `pageOrNotFound`. Числу страниц здесь по-прежнему неоткуда взяться, но
 * и незачем: страницы с номером сверх предела не существует.
 *
 * ⚠️ Инвариант держится ВЫЗЫВАЮЩИМИ, а не этой функцией. `generateMetadata` считается
 * независимо от `notFound()` в теле страницы, поэтому canonical для отказного адреса
 * всё равно вычисляется — просто его никто не увидит: ответ 404. Появится четвёртая
 * поверхность без `pageOrNotFound` — расхождение вернётся, и узда дисциплины листания
 * этого не поймает: она проверяет способ построения, а не наличие отказа.
 */
