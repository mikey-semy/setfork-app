 ...
 *
 * ⚠️ ЗАДАЁТСЯ В `.env` НА СЕРВЕРЕ — сервис `app` подключает его через `env_file`,
 * значение читается при старте контейнера. Второй путь, для того кто собирает свой
 * образ: `--build-arg NEXT_PUBLIC_SOURCE_URL=…`, тогда адрес вшивается в бандл.
 *
 * ⛔ ЧЕГО НЕЛЬЗЯ: экспортировать эту переменную в `ENV` Dockerfile «как три соседних»
 * и пробрасывать её `--build-arg` ПУСТОЙ. Next вшивает `NEXT_PUBLIC_*` литералом,
 * если при сборке переменная ОПРЕДЕЛЕНА; пустая вшивается как `("".trim()||"")` и
 * убивает чтение окружения. Тогда ссылка не появится ни одним способом.
 *
 * Так и было до 07.09.2026: `ENV X=$X` определяет переменную всегда, в том числе
 * пустой, — и в собранном образе стоял литерал вместо чтения. Поэтому `SOURCE_URL`
 * единственный из шести НЕ в `ENV`-цепочке Dockerfile; там же разобрано, почему.
 *
 * Проверять это надо в `.next` ВНУТРИ ОБРАЗА (`docker run --entrypoint sh`), а не в
 * локальном `npm run build`: локально переменной нет вовсе, в образе она приходила
 * из `ENV` пустой — и локальный замер показывал ровно обратное тому, что на проде.
 */
