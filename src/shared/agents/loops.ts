...
export const LOOPS: LoopSpec[] = [
  ...
  },
  {
    ...
    //
    // day.quiet — день без событий: для наблюдательной петли это СДЕЛАННАЯ работа, а не
    // пропуск. Отдельным действием, потому что «ok» у day.report значит «доставлено», и
    // размывать его тихим днём нельзя (второе замечание того же ревью).
    progress: ['day.report', 'day.quiet'],
