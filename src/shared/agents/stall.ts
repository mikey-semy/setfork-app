 ...
 * Находка A2 линзы 06.
 *
 * Список пуст только у петли, которой нет в реестре, а такого не бывает: единственный
 * вызывающий идёт по `AUTONOMOUS_LOOPS`, и контрактный тест держит эти два перечня равными.
 * Поэтому «пусто» здесь НЕ обрабатывается особо — если реестр однажды разъедется, детектор
