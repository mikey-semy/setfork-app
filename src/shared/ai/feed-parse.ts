...
/**
 * Снять CDATA и HTML-теги, свести пробелы: заголовки и описания в потоках приходят как попало.
 *
 * ТЕГИ СНИМАЕМ ДВАЖДЫ — и это не перестраховка. Половина лент отдаёт описание ЭКРАНИРОВАННЫМ
 * (`&lt;p&gt;текст`), и при одном проходе теги оживают уже ПОСЛЕ чистки: сначала мы вырезаем
 * настоящие теги, потом раскодируем сущности — и на выходе снова получаем разметку. Именно так
 * подсказка от kubernetes.io приезжала как «<p>Kubernetes ships with…» (проверено на живых
 * лентах 2026-07-28). Разметка в подсказке — мусор, который уедет в промпт и будет оплачен.
 *
 * `&amp;` раскодируем ПОСЛЕДНИМ: иначе `&amp;lt;` превратился бы в настоящий тег, то есть текст,
 * который автор экранировал НАМЕРЕННО, был бы выкинут как разметка.
 */
