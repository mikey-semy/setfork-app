/**
 * Заготовка из шаблона промпта вместо живого вопроса: модель иногда копирует
 * форму строки NEXT дословно — гость видел кнопки «q1 | q2 | q3» (фидбек
 * владельца). Такие «вопросы» не показываем: пустой список — это сигнал
 * поверхности показать свои универсальные направления.
 */
const isPlaceholder = (s: string) => /^[<{[][^>}\]]*[>}\]]$/.test(s) || /^(?:q|question|вопрос)\s*[-_]?\d+[.?]?$/i.test(s)
