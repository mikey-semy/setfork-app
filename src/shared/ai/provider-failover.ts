...
/**
 * Доступен ли провайдер: каталог отвечает и не пуст — и отвечает СЕЙЧАС, а не когда-то.
 * Кеш каталога живёт 10 минут, но для этой проверки он ограничен временем жизни решения:
 * иначе упавший основной провайдер ещё десять минут считался бы живым, и запасной не
 * включался бы вовсе.
 */
export async function providerReachable(provider: AiProviderId): Promise<boolean> {
  try {
    // Свежесть в пределах жизни решения: не чаще одного реального запроса в минуту на
    // провайдера, но и не старше — иначе «жив» означало бы «был жив десять минут назад».
    const r = await fetchModelsFor(provider, { maxAgeMs: DECISION_TTL_MS })
