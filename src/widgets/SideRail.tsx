...
export function SideRail({ lang, authed }: { lang: Lang; authed: boolean }) {
  ...
  return (
    <aside aria-label={t('menu', lang)} className="hidden w-[60px] shrink-0 border-r border-border bg-surface lg:block print:hidden">
      ...
      <nav className="sticky top-[53px] flex flex-col gap-0.5 px-1.5 py-2.5">
        {items.map((it) => {
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center gap-1 rounded-md px-0.5 py-1.5 text-center text-[9.5px] font-medium leading-tight ${
                active ? 'bg-surface-2 text-ink' : 'text-ink-2 hover:bg-surface-2 hover:text-ink'
              }`}
            >
              <it.icon size={18} className={active ? 'text-accent' : 'text-muted'} />
              {it.label}
            </Link>
          )
        })}
