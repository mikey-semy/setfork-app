...
export function GnomeLoader({ query, lang, label }: { query: string; lang: Lang; label?: string }) {
  ...
  // Мем-лоадер: форма отправлена, генерации ещё нет (её id появится после редиректа) — беседу
  // показывать неоткуда. Как только генерация создана, экран становится чатом (GenerationChat).
