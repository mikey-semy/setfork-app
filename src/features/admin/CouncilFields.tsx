...
// Поля КОМПАНИИ: совет, самогенерация, планка готовности. Живут на /admin/company/settings
// (submit через setCompanySettings) — раньше лежали внутри формы AI-настроек инстанса.
// Тумблеры — controlled Switch с name (submit 'on'/выкл), как в AiKeyAndSwitch.
//
// Три блока = три РАЗНЫХ решения владельца, поэтому и компонента три: страница кладёт
// каждый в свою карточку. Одна простыня на десяток полей плоха и на мобиле (длинный
// скролл без ориентиров), и по смыслу: «включить совет» и «публиковать без человека» —
// решения разного веса, принимаются в разное время.
...
/** Совет экспертов: кого зовём на генерацию и на каких моделях. */
export function CouncilBlock({ v, ru, modelOptions }: { v: CouncilValues; ru: boolean; modelOptions: Option[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* На узком экране два контрола в ряд сжимаются до нечитаемых — разводим по строкам. */}
      ...
      <Field
        ...
        hint={say(
          'When audience is Everyone: after N council uses a user falls back to single generation. Admins unlimited.',
          'Когда аудитория «Всем»: после N советов пользователь откатывается на одиночную генерацию. Админы — без лимита.',
        )}
      >
...
/**
 * САМОГЕНЕРАЦИЯ: инициатива компании, а не ответ на запрос пользователя. off — дефолт
 * (автономная трата не включается сама), manual — только кнопкой из состава, auto — петля
 * раз в 6 часов. Результат в любом режиме — черновик.
 */
export function SelfGenBlock({ v, ru }: { v: CouncilValues; ru: boolean }) {
  const say = (en: string, rus: string) => (ru ? rus : en) // строки-аргументы, не тернар-с-литералами (i18n-lint)
  return (
    <div className="space-y-3">
      <Field
        label={say('Mode', 'Режим')}
        hint={say(
      ...
      >
        <Select name="selfGenMode" defaultValue={v.selfGenMode}>
          ...
          <SelectContent>
            ...
            <SelectItem value="auto">{say('Auto — every 6 hours', 'Автоматически — раз в 6 часов')}</SelectItem>
          </SelectContent>
        </Select>
      </Field>
      {/* Оба числа обязаны быть В ФОРМЕ: экшен пишет ai.selfgen_* из formData, и поле,
          которого в форме нет, при сохранении уедет в минимум клампа. Так «за проход»
          молча схлопнулось бы в 1 и партия перестала бы работать. */}
      <div className="grid gap-3 sm:grid-cols-2">
 ...
 */
export function ReadinessBlock({ v, ru }: { v: CouncilValues; ru: boolean }) {
  const say = (en: string, rus: string) => (ru ? rus : en) // строки-аргументы, не тернар-с-литералами (i18n-lint)
  return (
    <div className="space-y-3">
      <Field
      ...
      <Field
        ...
        hint={say(
          'A safety quota: beyond it lists stay drafts even if they pass the bar. 0 = never publish automatically.',
