...
export function CouncilFields({ v, ru, modelOptions }: { v: CouncilValues; ru: boolean; modelOptions: Option[] }) {
  ...
  return (
    <div className="space-y-4 rounded-md border border-border bg-surface-2 p-3">
      <div>
        <div className="text-[0.8125rem] font-medium text-ink">{say('Expert council (multi-model)', 'Совет экспертов (мультимодельно)')}</div>
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="text-[0.8125rem] text-ink">{say('Enable council', 'Включить совет')}</div>
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="text-[0.8125rem] text-ink">{say('Elder web search (advanced)', 'Веб-поиск старейшины (advanced)')}</div>
          <p className="text-[0.78125rem] text-muted">{say('Also search the web for precedents (:online). Pricier & slower.', 'Искать прецеденты ещё и в интернете (:online). Дороже и медленнее.')}</p>
        </div>
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="text-[0.8125rem] text-ink">{say('Clarifying questions', 'Уточняющие вопросы')}</div>
          <p className="text-[0.78125rem] text-muted">{say('Ask before generating when the request is too vague.', 'Спрашивать перед генерацией, если запрос слишком расплывчатый.')}</p>
        </div>
