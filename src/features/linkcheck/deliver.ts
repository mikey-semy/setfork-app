  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        // Раздел «Вопросы» выключен — задачу тут заводить некуда, и незачем ходить за
        // ней каждый обход: ядро всё равно откажет, а в журнал ляжет строка отказа.
        // Это ОПТИМИЗАЦИЯ, а не право: проверка в ядре остаётся вторым рубежом (иначе
        // право зависело бы от того, не забыл ли отбор про условие). Самовосстановление
        // сохраняется — фильтр считается заново на каждом обходе: включили вопросы, и
        // список снова в выборке.
        eq(templates.issuesEnabled, true),
      ),
    )
