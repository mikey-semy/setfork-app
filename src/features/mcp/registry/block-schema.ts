...
/**
 * Схема блока списка — ОДНА на все инструменты записи.
 *
 * Её просят и `create_list`/`update_list`/`patch_list`, и `suggest_edit`: набор
 * полей у блока один, и разъехаться описаниям нельзя — агент читает их как
 * контракт. Раньше схема жила посреди файла транспорта, между советом гномов и
 * списками, и «общей» была лишь по факту соседства.
 */
