...
/**
 * ОБЛЕГЧЁННАЯ ФОРМА БЛОКА — та же схема, но без полей редких типов.
 *
 * ⚠️ Это экономия ЧУЖИХ токенов, а не наше удобство. Список инструментов уходит в
 * контекст агента при КАЖДОМ запросе; замер 19.09.2026: 37 инструментов = 16 254
 * токена, из них схемы — 11 080. Полная форма блока весит 1114 токенов и вписана в
 * ПЯТЬ пишущих инструментов; ссылки ($ref/$defs) тут не помогают — они действуют
 * внутри одной схемы, а у нас повтор между разными.
 *
 * Что оставлено — решено по живому корпусу, а не по вкусу (прод, 19.09.2026):
 *   step 73,87 %, text 24,08 %, quiz 2,05 %, а image/poll/video/file — НОЛЬ блоков.
 * Поля теста стоят 389 токенов, поля четырёх неиспользуемых типов — 209: больше
 * половины схемы описывало то, чем не пользуются, и повторялось пятикратно.
 *
 * ⚠️ ВОЗМОЖНОСТИ НЕ УБАВИЛОСЬ, И ЭТО ГЛАВНОЕ. Форма `passthrough`: поля редких типов
 * по-прежнему ПРИНИМАЮТСЯ, обработчик разбирает их полной `itemShape`, и ничего не
 * теряется молча (проверено мутацией — снятый `passthrough` роняет тест на блоке-тесте).
 * Изменилось только то, что агент видит в списке инструментов: полная форма живёт в
 * `patch_list`, и описания говорят, где её взять.
 */
export const itemShapeLean = itemShape
  .pick({ type: true, bid: true, section: true, title: true, desc: true, command: true, level: true, why: true, subtasks: true, refs: true, needsHuman: true, needsHumanAsk: true, text: true })
  .passthrough()
