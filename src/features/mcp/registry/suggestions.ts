import { z } from 'zod'
import { mcpApplySuggestion, mcpMergeSuggestion, mcpPendingSuggestions, mcpReviewSuggestion, mcpRevertSuggestion, mcpSuggestEdit } from '@/features/mcp/tools'
import { itemShape } from './block-schema'
import { err, json, type ToolKit } from './kit'

/** Предложения правок: очередь, применение, ревью, слияние, откат. */
export function registerSuggestionTools({ readTool, writeTool }: ToolKit) {
  readTool(
    'pending_suggestions',
    {
