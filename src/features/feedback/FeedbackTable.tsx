...
function StatusBadge({ status, lang }: { status: FeedbackItem['status']; lang: Lang }) {
  ...
  return <span className={`rounded-full px-2 py-0.5 text-[11.5px] font-semibold ${cls}`}>{t(label, lang)}</span>
}
  ...
  return (
    <div className="rounded-lg border border-border bg-surface p-4">
      <div className="mb-2 flex flex-wrap items-center gap-2 text-[12.5px] text-ink-2">
        ...
        <span className="rounded-full bg-surface-2 px-2 py-0.5 text-[11.5px] font-semibold text-ink-2">
      ...
      {item.pageUrl && <p className="mt-2 break-all text-[11.5px] text-muted">{item.pageUrl}</p>}
