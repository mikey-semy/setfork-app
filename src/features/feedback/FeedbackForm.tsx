  ...
  if (state?.ok) {
    return (
      <div className={cardClass({ pad: 'lg', className: 'text-center' })}>
        <div className="mb-1 text-title font-bold text-ink">{t('fbThanks', lang)}</div>
        <p className="text-body text-ink-2">{t('fbThanksBody', lang)}</p>
      </div>
    )
  }
  ...
  return (
    <form action={action} className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1.5">
        <span className="text-body-sm font-semibold text-ink-2">{t('fbCatLabel', lang)}</span>
