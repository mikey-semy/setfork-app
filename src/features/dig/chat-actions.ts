...
export async function digChatAsk(input: {
  ...
  // Теневой вопрос о ремесле на ответ НЕ влияет, поэтому ответ его не ждёт: зависни
  // Decisions — человек ждал бы до десяти секунд ради записи для калибровки (авто-ревью к
  // #964). Запись идёт в фоне, как сохранение беседы ниже; ошибки тень гасит сама.
  void guide?.shadow
