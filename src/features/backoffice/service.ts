...
export async function runChronicleSweep(): Promise<ChronicleResult> {
  ...
  const html = `<p>День компании, ${date}:</p><ul>${rows.map(([k, n]) => `<li>${k}: ${n}</li>`).join('')}</ul>` +
    ...
    //
    // Формулировка правится по замечанию авто-ревью (#775): счёт идёт по ВСЕМ вызовам, а не
    // только по вызовам компании — отличить их нечем, в журнале расхода такого признака
    // нет. Прежний текст объявлял компанию виновной в чужих отказах: пять неудачных
    // пользовательских генераций читались как «компания не сделала ничего». Теперь сводка
    // говорит ровно то, что измерено, а вывод про компанию идёт следствием.
    (broken
      ? `<p><b>За день не прошёл ни один вызов модели: ${day1.failed} из ${day1.calls} с отказом.</b> Похоже на поломку канала или на исчерпанный бюджет — и компания в такой день ничего сделать не могла.</p>`
      : '') +
