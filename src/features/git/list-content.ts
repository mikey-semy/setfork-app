...
export type WireStep = {
   ...
   *  и принимает на записи. Раньше `toWireContent` их не собирал: считалось, что
   *  ядро возьмёт их из текущей версии по blockId. Оно и берёт — НО ТОЛЬКО ДЛЯ
   *  БЛОКА, КОТОРЫЙ В ТЕКУЩЕЙ ВЕРСИИ ЕСТЬ. У блока, заведённого прямо в ветке, его
   *  там нет по определению: переносить нечего, и пометка тихо пропадала. */
  danger?: boolean
  imageKey?: string
  needsHuman?: boolean
...
export function toWireContent(c: ListContent): WireContent {
  return {
    ...
    steps: c.steps.map((s) => ({
      ...
      // Пометки блока идут на провод ЯВНО. Перенос из текущей версии по blockId,
      // на который здесь рассчитывали раньше, спасает только блок, который в той
      // версии есть; у нового блока ветки его нет, и пометка терялась при слиянии.
      //
      // Снятое остаётся пустым, а не превращается в «снять»: ядро пишет в канон
      // только непустое, а отсутствие в каноне оно читает как «не задано» и
      // переносит прежнее. Так добавление значения не может стереть чужую пометку.
