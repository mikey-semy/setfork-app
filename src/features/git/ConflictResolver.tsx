...
function StepCard({ s, deleted, ru }: { s: TwStep | null; deleted: string; ru: boolean }) {
  if (!s) return <div className="px-3 py-2 text-body-sm italic text-muted">{deleted}</div>
  return (
    <div className="min-w-0 px-3 py-2">
      ...
      {s.why && <div className="mt-0.5 truncate text-body-sm text-muted">{ru ? 'зачем: ' : 'why: '}{s.why}</div>}
    </div>
  )
}
          ...
          <div key={m.field}>
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              <Side label={theirsLabel} tone="theirs" selected={metaChoices[m.field] === 'theirs'} onSelect={() => setMetaChoices((c) => ({ ...c, [m.field]: 'theirs' }))}>
                <div className="px-3 py-2 text-body-sm text-ink-2">{JSON.stringify(m.theirs)}</div>
              </Side>
            </div>
          </div>
        ))}
