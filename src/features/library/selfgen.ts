...
export async function selfGenerateOne(expertId: string, topicOverride?: string): Promise<SelfGenResult> {
  ...
  // РОЖДЁННЫЙ ИЗ НОВОСТИ — ЖИВОЙ. Тема, выросшая из события, продолжает развиваться: по ней
  // придёт следующая новость, и список должен расти, а не «устояться». Тема из головы живой не
  // становится — там расти нечему. Владелец может снять признак в настройках списка: угадали
  // мы или нет, решает человек, а не петля.
  if (fresh) await db.update(templates).set({ living: true }).where(eq(templates.id, created.id))
