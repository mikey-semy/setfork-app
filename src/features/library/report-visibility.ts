/**
 * ПОКАЗ ОТЧЁТА О ПРОГОНЕ: что видно и как это выглядит строкой.
 *
 * Модуль намеренно БЕЗ `server-only`. Здесь только чистые правила показа — их зовёт и
 * серверная разметка, и (при необходимости) клиентская: `RunReportLine` — серверный
 * компонент, но одно `'use client'` в нём уронило бы сборку у того, кто его добавит,
 * а не у того, кто писал импорт.
 */
