 ...
 *
 * ⚠️ ОСТАЮЩИЙСЯ ЗАЗОР, ОСОЗНАННО. Условие закрывает случай «ядро записало ДО
 * нашего дедлайна». Обратный — ядро дописывает ПОСЛЕ, когда мы уже посчитали, —
 * локально не закрывается ничем: чтобы его снять, нужен либо сквозной
 * идентификатор попытки через proto, либо отказ от записи здесь вообще. Второе
 * возвращает дыру пострашнее (зеркало, настроенное при лежащем ядре, не
 * попадает в повторы никогда), первое — правка контракта ради разницы в один
 * шаг лестницы пауз. Цена зазора: редкая гонка сдвигает паузу на одну ступень,
 * и первый же успешный пуш обнуляет счётчик. Оставлено сознательно
