...
/** Держит ли блок ссылки-источники (`refs`). Шаг и текст (#962).
 *
 *  Хранилище и канон держат `refs` у ЛЮБОГО блока, но показ и форма правки есть
 *  только у этих двух — у остальных ссылки не видны и стёрлись бы первым же
 *  сохранением в редакторе. Отсюда читает запись через MCP: ссылки блоку другого
 *  типа — отказ с понятным текстом, а не молчаливая потеря. */
export const carriesRefs = (t: string): boolean => t === 'step' || t === 'text'
