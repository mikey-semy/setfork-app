...
export function toMarkdown(list: ExportList, lang: Lang): string {
  ...
  list.steps.forEach((s) => {
    ...
    // ⚠️ ОПАСНОСТЬ ШАГА — в тексте, а не только в скрипте. Этот markdown читает и агент
    // (ресурс MCP `setfork://lists/…` отдаёт ровно его), и по нему он ИСПОЛНЯЕТ шаги.
    // Без пометки агент, получивший список ресурсом, видел бы `rm -rf` как обычный шаг,
    // хотя `get_list` про ту же команду предупреждает (`danger`/`dangerHint`). Правило
    // одно с `get_list` и скриптом — `stepDanger`: пометка автора или находка детектора.
    // Причину не пишем: у части ключей детектора (набор RISKY) текста в словаре нет, а
    // пометка отвечает на один вопрос — «исполнять ли молча», как и в скрипте.
    if (s.command && stepDanger(s)) out.push(`${indent}> ⚠ Destructive step: review the command before running it.`)
