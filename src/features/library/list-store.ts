 ...
 * причине: редактор кладёт поля одним языком, и правило «не тронул — сохрани
 * прежнее целиком» пришлось бы помнить в каждом из путей записи (форма,
 * черновик, предложение правки, MCP, садовник). Забытое в одном из них, оно
 * молча стирает переводы всего списка.
 *
 * Сопоставление идёт с ТЕКУЩЕЙ версией: именно её показывал редактор.
 */
async function withCarriedTranslations(templateId: string, steps: NewStepInput[]): Promise<NewStepInput[]> {
  const [tpl] = await db
