    ...
    <div className={row}>
      <div className="flex min-w-0 items-start gap-2.5">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-body-sm leading-snug text-ink-2">{hint}</p>
        </div>
      </div>
      <Switch
        checked={on}
...
function WhoRow({ templateId, initial, lang }: { templateId: string; initial: 'all' | 'collaborators'; lang: Lang }) {
  ...
  return (
    <div className={row}>
      <div className="min-w-0">
        <div className="text-body font-medium text-ink">{t('prSetWho', lang)}</div>
        <p className="mt-0.5 text-body-sm leading-snug text-ink-2">{t('prSetWhoHint', lang)}</p>
      </div>
      <Select
        value={val}
        onValueChange={(v) => {
  ...
  return (
    <div className={row}>
      <div className="flex min-w-0 items-start gap-2.5">
        ...
        <div className="min-w-0">
          <div className="text-body font-medium text-ink">{t('prSetMethod', lang)}</div>
          <p className="mt-0.5 text-body-sm leading-snug text-ink-2">{t('prSetMethodHint', lang)}</p>
        </div>
      </div>
      <Select
        value={val}
...
function ApprovalsRow({ templateId, initial, lang }: { templateId: string; initial: number; lang: Lang }) {
  ...
  return (
    <div className={row}>
      <div className="min-w-0">
        <div className="text-body font-medium text-ink">{t('prSetApprovals', lang)}</div>
        <p className="mt-0.5 text-body-sm leading-snug text-ink-2">{t('prSetApprovalsHint', lang)}</p>
      </div>
      <Select
        value={String(val)}
        onValueChange={(v) => {
