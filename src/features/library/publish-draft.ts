    ...
    const [wrote] = await db
      ...
      .where(
        and(
          ...
          // Владелец — в условие наравне со статусом и видимостью. Список успевает уехать
          // по передаче прав, пока пачка идёт: получатель принял передачу, а прежний
          // владелец этим же запросом опубликовал бы уже ЧУЖОЙ черновик (находка авто-ревью).
