...
export async function getFeed(
  ...
  // запросе не выражается. Безразмерным он от этого не становится — но и обрезать
  // запрошенное не должен: берём столько, сколько нужно ЗАПРОСИВШЕМУ (страница плюс её
  // смещение), и не меньше потолка настроек. Иначе «дай 50» тихо отдавало бы 20, а вторая
  // страница поиска оказалась бы пустой (находка авто-ревью).
  const need = window ? (window.offset ?? 0) + window.limit : 0
  const cap = { limit: Math.max(limit, need) }
