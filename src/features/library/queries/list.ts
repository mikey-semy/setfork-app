...
/**
 * КЛЮЧ СПИСКА — ровно те поля, по которым выдачу отбирают и упорядочивают.
 *
 * Нужен, чтобы страница не таскала корпус ради экрана. Профиль отбирает и сортирует
 * свою библиотеку в приложении (поиск по названию, тип, полка, порядок), поэтому весь
 * подходящий набор ему знать надо — а вот `desc`, теги, счётчики, автор и подписанные
 * адреса картинок нужны ТОЛЬКО двадцати показанным строкам. Раньше ехало всё и на всех:
 * у владельца это 518 полных строк с join'ом на автора и тысяча с лишним подписей
 * картинок на каждый показ профиля.
 *
 * Это обычный «поздний доступ к строке»: сначала дёшево отобрать нужные id, потом
 * дорого достать только их (`getTemplatesByIds`).
 */
