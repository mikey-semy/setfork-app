...
/**
 * ЧТО МЫ ПРЕДЛАГАЕМ МАШИНАМ КАК ГОТОВОЕ — одно правило на карту сайта и на llms.txt.
 *
 * Видимость («кому можно показать») и индексация («что мы выдаём за готовое») — разные
 * вопросы, и решение 0018 разводит их прямо: список уровня «порода» публиковать можно,
 * но в витрину и в индекс он не идёт до первой проверки.
 *
 * Правило живёт ЗДЕСЬ, а не копией в каждом файле-выгрузке: карта сайта, прячущая
 * породу, и llms.txt, её рекламирующий, — это противоречие в один шаг, и заметить его
 * было бы некому. Оба адресата машинные, и оба читают наши утверждения о готовности.
 *
 * ⚠️ «Нет строки версии» ≠ «порода»: порода — сказанное утверждение, отсутствие версии —
 * аномалия данных. Приравнять их значило бы молча выкинуть неизвестное число списков
 * под видом правила. Исключается ТОЛЬКО явная порода.
 */
export function indexableFilter(): SQL {
  // ⚠️ УРОВНЯ ПРОВЕРКИ ЗДЕСЬ НЕТ, и это следствие живого инцидента (31.08): гейт по
  // незаполненной колонке выкинул из карты сайта ВЕСЬ корпус — уровни проставлены
  // примерно у полупроцента списков. Правило по пустому полю не фильтрует, а обнуляет.
  //
  // Функция сохранена именно ради будущего гейта: когда уровни будут заполнены и решение
  // вернётся с порогом по числам, условие добавится ЗДЕСЬ — и подействует разом на карту
  // сайта и на llms.txt, а не в одной копии из двух.
