...
// видна сразу.
//
// И поэтому же перенос применяется НЕ ко всякой записи. Правило «не изменилось —
// верни прежнее» не отличает «человек не трогал поле» от «запись осознанно
// убрала язык»: в обоих случаях присланное поле выглядит одинаково. Отличить их
// по данным нельзя, можно только по намерению — его и объявляет `langScope`
// (см. ProposedItem). Ставит его одноязычный редактор; MCP, перевод и импорт
// распоряжаются всем LocaleText сами, метки не ставят и сюда не попадают.
//
// Это не предусмотрительность, а исправление: сначала перенос стоял на всякой
// записи, и `patch_list`, умеющий убрать один язык и оставить второй, начал
// получать убранный язык обратно. Поймал интеграционный тест.
