 ...
 * Окно тоже другого порядка: между чтением снимка и записью нет ни модели, ни
 * человека — доли секунды внутри самого экшена, а номер версии в форму не уезжает
 * (страница истории перечитывает его на каждом рендере).
 */
