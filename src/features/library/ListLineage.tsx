...
export function ListLineage({ lineage, exact, gnomeNames, lang }: { lineage: Lineage; exact: boolean; gnomeNames?: Record<string, string>; lang: Lang }) {
  return (
    <div className="mt-4 border-t border-border pt-3">
      ...
      <p className="text-body-sm text-muted [overflow-wrap:anywhere]">
      ...
      {lineage.rejected.length > 0 && (
        <div className="mt-2">
          <span className="text-caption font-semibold text-ink-2">{t('library.notChosen', lang)}:</span>
          <ul className="mt-1 flex flex-col gap-1">
            {lineage.rejected.map((r) => (
              <li key={r.idx} className="min-w-0 text-body-sm text-muted [overflow-wrap:anywhere]">
