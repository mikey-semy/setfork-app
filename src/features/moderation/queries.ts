...
export type ModFilter = 'all' | 'flagged' | 'hidden'
...
export async function getModerationCounts(): Promise<{ flagged: number; hidden: number }> {
  const [r] = await db
    .select({
    ...
    .from(templates)
  return { flagged: r?.flagged ?? 0, hidden: r?.hidden ?? 0 }
}
