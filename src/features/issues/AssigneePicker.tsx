...
export function AssigneePicker({
  ...
  return (
    <div className="flex flex-col gap-2">
        ...
        {canEdit && (
          ...
          >
            {(close) => (
              ...
              >
                {found.length === 0 ? (
                  <div className="px-2 py-3 text-body-sm text-muted">{L('начните вводить handle', 'start typing a handle')}</div>
                ) : (
                  found.map((u) => (
                    <PickerRow
                      key={u.handle}
      ...
      {assignees.length === 0 ? (
        <span className="text-body text-muted">{labels?.empty ?? L('никого', 'no one')}</span>
      ) : (
