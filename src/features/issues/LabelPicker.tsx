      ...
      {ISSUE_LABELS.map((l) => {
        ...
        return (
          <button
            ...
            className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[12px] transition-colors ${on ? l.cls : off}`}
          >
            {on && <Check size={11} />}
            {lang === 'ru' ? l.ru : l.en}
          </button>
      ...
      {custom.map((c) => {
        ...
        return (
          <button
            ...
            className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[12px] transition-colors ${on ? '' : off}`}
          >
