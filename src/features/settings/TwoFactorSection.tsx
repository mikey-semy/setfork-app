...
export function TwoFactorSection({ enabled, lang }: { enabled: boolean; lang: Lang }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2">
        {isOn ? (
          <>
            ...
            <span className="text-[13.5px] font-semibold text-ink">{ru ? 'Двухфакторная защита включена' : 'Two-factor is enabled'}</span>
      ...
      {err && <div className="rounded-md border border-danger/40 bg-danger/10 px-3 py-2 text-[13px] text-danger">{err}</div>}
