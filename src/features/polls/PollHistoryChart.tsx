...
export function PollHistoryChart({ events, options, lang }: { events: PollHistoryEvent[]; options: { id: string; text: string }[]; lang: Lang }) {
  const ru = lang === 'ru'
  if (!events.length) return <p className="py-2 text-center text-body-sm text-muted">{ru ? 'Пока нет голосов' : 'No votes yet'}</p>
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <p className="text-caption text-muted">{ru ? 'Резкий скачок одной линии может указывать на накрутку.' : 'A sharp jump in one line may indicate vote manipulation.'}</p>
    </div>
  )
}
