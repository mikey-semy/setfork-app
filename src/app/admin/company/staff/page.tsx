 ...
 * каждого — на его собственной странице (/admin/company/staff/<id>), как у списка на странице списка.
    ...
    <div className="flex w-full min-w-0 flex-col gap-4 px-5 py-6 md:px-8">
      {settings.councilEnabled ? null : (
        <p className="mb-4 rounded-md border border-warn/50 bg-surface px-3 py-2 text-[12.5px] text-warn">
          {say(
            'The council is off — these experts are not summoned. Turn it on in Company → Company settings.',
            'Совет выключен — этих экспертов никто не зовёт. Включается в «Компания» → «Настройки компании».',
          )}
        </p>
      )}
      {sp.hire === 'failed' && (
