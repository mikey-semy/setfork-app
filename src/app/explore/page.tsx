  ...
  return (
    <div className="mx-auto flex w-full max-w-[1280px] flex-1 items-stretch">
      <aside className="hidden w-[240px] flex-shrink-0 border-r border-border bg-surface-2 px-3 py-5 md:block">
        <ScopeSwitcher active={scope} counts={counts} q={sp.q} sort={sp.sort} lang={lang} />
        {/* List-специфичные фасеты — только для scope=lists */}
        {scope === 'lists' && <AdvancedFacets initialQ={sp.q ?? ''} tags={tags} lang={lang} />}
      </aside>
      ...
      <section className="min-w-0 flex-1 px-6 py-4">
        ...
        {scope === 'lists' && (
          <>
            {/* Нечего найти по теме → предложить сгенерировать СПИСОК (только по свободному
                тексту; по фильтрам людей/тегов — нет, мы генерируем списки, не пользователей). */}
