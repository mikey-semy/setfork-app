  ...
  return (
    <div className={PAGE_NARROW}>
      ...
      <form action={action}>
        ...
        {/* ⚠️ ЗАМЕТКА И НАСТРОЙКИ — ОДНИМ РЯДОМ. Действие НАД содержимым поля живёт
            внутри поля справа (генерация заметки), настройка САМОГО списка — снаружи
            справа от него. Кнопка настроек стояла отдельной строкой ниже: на телефоне
            это читалось как ещё одно поле формы, а не как её свойство (снимок
            владельца 01.09.2026). На «Создать список» она и была рядом с полем —
            здесь просто разъехалось.

            `items-center` — как в шапке редактора (`headerField`/`headerRight`): иначе
            32px кнопка встала бы на 8px выше центра 40px поля, и два экрана, которые
            эта правка и сводит к одному виду, всё равно смотрелись бы по-разному. */}
        <div className="mb-6 flex items-center gap-2">
          <ChangeNoteField templateId={tpl.id} lang={lang} placeholder={t('changeNote', lang)} required={false} initial={draft?.note ?? ''} className="mb-0 flex-1" />
