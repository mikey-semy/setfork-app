    ...
    <>
      ...
      <FloatingBack href={`/${owner}/${slug}/discussions`} label={t('featDiscussions', lang)} />
        ...
        <NewDiscussionForm emptyText={t('discussionTitleRequired', lang)} className="flex flex-col gap-3">
          ...
          <div>
            <div className="mb-1.5 text-body-sm font-semibold text-ink-2">{ru ? 'Категория' : 'Category'}</div>
            <div className="flex flex-wrap gap-2">
              {DISCUSSION_CATEGORIES.map((c, i) => (
                <label
                  key={c.key}
                  className="flex cursor-pointer items-center gap-1.5 rounded-md border border-border bg-surface-2 px-3 py-1.5 text-body text-ink has-checked:border-accent has-checked:bg-accent-soft"
                >
