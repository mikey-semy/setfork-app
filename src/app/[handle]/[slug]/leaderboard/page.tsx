          ...
          <ol className="flex flex-col divide-y divide-border overflow-hidden rounded-lg border border-border bg-surface">
            {rows.map((r, i) => (
              <li key={r.handle} className="flex items-center gap-3 px-4 py-2.5">
                <span className="w-7 shrink-0 text-center text-[14px]">{i < 3 ? medal[i] : <span className="font-mono text-[12.5px] text-muted">{i + 1}</span>}</span>
                <Link href={`/${r.handle}`} className="flex min-w-0 flex-1 items-center gap-2.5 hover:opacity-90">
                  ...
                  <span className="min-w-0">
                    <span className="block truncate text-[13px] font-medium text-ink">{r.name ?? r.handle}</span>
                    <span className="block truncate text-[12.5px] text-ink-2">{r.handle}</span>
                  </span>
                </Link>
